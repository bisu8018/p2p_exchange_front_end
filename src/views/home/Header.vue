                ...
                <v-flex xs12 v-if="drawer" @click.stop="drawer = !drawer">
                    <v-card style="position: relative; z-index:100;" class="BGNav">
                        <v-layout row wrap >
                            ...
                            <!-- OTC버튼-->
                            <v-flex xs12 text-xs-left mt-4 ml-3><div class="button-2 " style="color: #ffffff;" right flat  @click="goTradeCenter()">OTC</div></v-flex>
                            <!-- Exchange 버튼-->
                            <v-flex xs12 text-xs-left mt-4a ml-3><div class="button-2" style="color: #ffffff;  " flat >{{$str("exchange")}}</div></v-flex>
                            <!-- post AD 버튼 -->
                            <v-flex xs12 text-xs-left mt-4a mb-3 >
                                <div class="button-2" style="color: #ffffff; " flat @click.stop="postadDrawer = !postadDrawer"><div class="ml-3">{{$str("postAd")}}</div>
                                    <!--post AD 눌렀을때 나오는 세부항목-->
                                    <div v-if="postadDrawer" style=" background-color: #21407e; ">
                                        <v-flex xs12 text-xs-left mt-3  ml-5 pt-3><div class="button-2" style="color: #ffffff; " flat >{{$str("Post General AD")}}</div></v-flex>
                                        <v-flex xs12 text-xs-left mt-4a  ml-5 pb-3><div class="button-2" style="color: #ffffff; " flat >{{$str("Post Block AD")}}</div></v-flex>
                                    </div>
                                </div>
                            </v-flex>
                            <!-- login 버튼-->
                            <v-flex xs12 text-xs-left pt-3 ml-3><div class="button-2" style="color: #ffffff; "  flat  @click="goLogin()">{{$str("loginText")}}</div></v-flex>
                            <!-- signup 버튼-->
                            <v-flex xs12 text-xs-left mt-4a mb-4 ml-3><div class="button-2" style="color: #ffffff;" flat  @click="goSignup()">{{$str("signupText")}}</div></v-flex>
                        </v-layout>
                    </v-card>
                </v-flex>
            </div>
